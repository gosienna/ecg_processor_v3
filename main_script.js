...
let seg_head=0
let seg_tail=0
//array to collect vertical marker location(peak of ecg wave)
let peak_info=[]
...
function init_ecg(ecg_data){    
    ...
    canvas_ID_list.forEach(function(id){
        ...
        Plot2D_obj.add_vertical_marker() // activate moving vertical marker
        ...
        document.getElementById(id).addEventListener('click',function(event){
            ...
            x=parseInt(x)
            let wave_type=document.getElementById('wave_type').value
            //function to call out add_V_line function in the canvas object
            canvas_obj_list.forEach(function(Plot2D_obj){
                Plot2D_obj.add_V_line(x,wave_type)
            })
            peak_info.push([x,wave_type])
            // console.log(peak_info)
        })
        //--------------segment marking---------------------
        // document.getElementById(id).addEventListener('click',function(event){
        //     click=!click
...
document.getElementById('save').addEventListener('click',function(){
    ...
    let peak_type = 'null'
    for(let i = 0; i < ecg_data.lead_I.length; i++){
        canvas_ID_list.forEach(function(ID){
            result += ecg_data[ID][i] 
            result += ","
        })
        //screen through collected peak information
        peak_info.forEach(function(e){
            if(i=== e[0]){
                peak_type = e[1]
            }
        })
